...
| **Tool migrasi + tabel versi** | Migrasinya sendiri ada — `db/migrations/` berisi SQL bernomor yang dijalankan manual (`001_zona.sql`, `002_kode.sql`). Yang tidak ada adalah kerangka di atasnya: tidak ada Alembic, dan tidak ada tabel versi yang mencatat migrasi mana yang sudah jalan. Menjalankan ulang sebuah migrasi gagal di kolom duplikat — itulah satu-satunya pengaman idempotensi, dan pengaman yang aman: menolak, bukan setengah jalan. Setiap migrasi juga dilipat ke `db/schema.sql` supaya build baru cocok dengan database yang sudah dimigrasi. |
